...
const questionFormSchema = z.object({
  content: z
    .string()
    .trim()
    .min(1, 'Escreva sua pergunta antes de enviar')
    .max(400, 'A pergunta deve ter no máximo 400 caracteres'),
});
...
export function Room() {
  ...
  const newQuestion = watch('content') ?? '';

  async function handleSendQuestion({ content }: QuestionFormData) {
    if(!user) {
      return;
    }
  ...
  return (
    <div id="page-room">
      ...
      <main>
        ...
        <form onSubmit={handleSubmit(handleSendQuestion) as (event: FormEvent) => void}>
          <textarea
            ...
            >
              ...
              {!question.isAnswered && (
                <button
                  ...
                  disabled={!user}
                  title={user ? 'Marcar como gostei' : 'Entre para curtir'}
